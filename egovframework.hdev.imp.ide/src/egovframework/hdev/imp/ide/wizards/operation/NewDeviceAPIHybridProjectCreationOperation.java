 ...
 * @ @ 수정일 수정자 수정내용 @ ------------- --------- ------------------------------- @
 *   2012. 08. 22. 이율경 최초생성 @ 2013. 10. 11. 이기하 classpathentry 설정변경(exported
 *   true)
 * 
...
public abstract class NewDeviceAPIHybridProjectCreationOperation extends NewDeviceAPIWebProjectCreationOperation {
	...
	protected void updateAndroidNature(IProgressMonitor monitor) throws CoreException {
		...
		DeviceAPIIdeUtils.addNatureToProject(getDeviceapiProject(), ProjectFacetConstants.ANDROID_NATURE_ID, monitor);
	}

	/**
	 * 기본 리소스 생성
	 */
	protected void createAndroidDefaultResource(IProgressMonitor monitor) throws CoreException {

		for (int i = 0; i < ResourceConstants.ANDROID_SYSTEM_FOLDERS.length; i++) {
			org.eclipse.core.resources.IFolder folder = getDeviceapiProject()
					.getFolder(ResourceConstants.ANDROID_SYSTEM_FOLDERS[i]);
	...
	protected void addAndroidProjectFacets(IProgressMonitor monitor, IProject project) throws CoreException {
	 ...
	 * @throws IOException
	 */
	protected void createDeviceapiPomFile(IProject project, IProgressMonitor monitor) throws CoreException {
		...
		try {
			...
			String document = stream2string(inputStream, context.getGroupId(), context.getArtifactId(),
					context.getVersion(), context.getDeviceapiPackageName());
	...
	protected void updateDeviceapiServerInfoFile(IProject project, IProgressMonitor monitor)
		...
		IFile file = project.getFile(
				ResourceConstants.DEVICEAPI_SERVER_INFO_PATH.append(ResourceConstants.DEVICEAPI_SERVER_INFO_FILENAME));
		if (file.exists()) {
		...
		try {
			...
			String document = stream2ServerInfoString(inputStream, context.getServerURL());
			stream = new ByteArrayInputStream(document.getBytes());
	...
	protected InputStream openAndroidPomContentStream() {
		return getClass().getClassLoader()
				.getResourceAsStream(ResourceConstants.ANDROID_POM_EXAMPLE_PATH + context.getDeviceapiPomFileName());
	}
	...
	protected InputStream openDeviceapiServerInfoContentStream() {
		return getClass().getClassLoader().getResourceAsStream(ResourceConstants.DEVICEAPI_SERVER_INFO_EXAMPLE_PATH
				+ ResourceConstants.DEVICEAPI_SERVER_INFO_FILENAME);
	}
	 ...
	 * @throws IOException
	 */
	protected String stream2ServerInfoString(InputStream stream, String serverURL) throws IOException {
	 ...
	 * 자바 1.6 클래스 패스삭제
	 */
	protected void removeJavaClasspath(IProgressMonitor monitor) throws CoreException {
