...
/**
 * 
 * 컴포넌트 정보가 담긴 엑셀파일을 읽어서 지정된 위치에 위저드가 읽을 수 있는 컴포넌트 정보 파일을 temp 파일로 생성한다. 개발자는
...
public class CreateComponentInfo {
	...
	public static void main(String[] args) throws Exception {
		try {
			...
			// xml 파일
			File xmlFile = File.createTempFile("components", ".xml", new File(generateXMLFilePath));
			FileWriter xmlWriter = new FileWriter(xmlFile);
			xmlWriter.write("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n");
			xmlWriter.write("<components>\n");

			// java 파일
			File javaFile = File.createTempFile("EgovComponentsInfo", ".java", new File(generateFilePath));
			FileWriter javaWriter = new FileWriter(javaFile);
			javaWriter.write("package egovframework.dev.imp.commngt.common;\n\n");
			javaWriter.write("import org.eclipse.osgi.util.NLS;\n\n");
			javaWriter.write("public class EgovComponentsInfo extends NLS {\n");
			javaWriter.write(
...
//        	InputStream en_inp = new FileInputStream(readFilePath_en);
//			HSSFWorkbook en_wbT =  new HSSFWorkbook(en_inp);
//        	HSSFSheet en_sheetT = en_wbT.getSheet(en_wbT.getSheetName(0));
			...
			for (int i = 2; i <= sheetT.getLastRowNum(); i++) {
				...
				if (!category_nam.equals((cellValue1 + "").trim())) {
					...
					xmlWriter.write("\t<category name=\"#category" + catNum + "nam#\" description=\"#category" + catNum
							+ "des#\" >\n");
					// java 쓰기
				...
				if (category_Component_use == null || category_Component_use.equals(""))
					category_Component_use = "N/A";
				if (category_Component_web.equals("Y"))
					category_Component_web = "true";
				else if (category_Component_web.equals("N"))
					category_Component_web = "false";
				if (category_Component_add.equals("Y"))
					category_Component_add = "true";
				else if (category_Component_add.equals("N"))
					category_Component_add = "false";
				...
				category_Component_nam_en = (cellValue13 + "").trim().replace("\n", " ").replace("(", "&#40;")
						.replace(")", "&#41;");
				category_Component_des_en = (cellValue14 + "").trim().replace("&", "&#38;").replace("(", "&#40;")
						.replace(")", "&#41;").replace("-", "&#45;").replace("/", "&#47;").replace("\n", "<br/>");
				...
				// properties 쓰기
				propertiesKoWriter
						.write("category" + catNum + "Component" + comNum + "nam=" + category_Component_nam + "\n");
				propertiesKoWriter
						.write("category" + catNum + "Component" + comNum + "pac=" + category_Component_pac + "\n");
				// properties 영문 쓰기
				propertiesEnWriter
						.write("category" + catNum + "Component" + comNum + "nam=" + category_Component_nam_en + "\n");
				propertiesEnWriter
						.write("category" + catNum + "Component" + comNum + "pac=" + category_Component_pac + "\n");
				// xml 쓰기
				xmlWriter.write("\t\t<component name=\"#" + "category" + catNum + "Component" + comNum + "nam#\" >\n");
				xmlWriter.write("\t\t\t<packageName      >#" + "category" + catNum + "Component" + comNum
						+ "pac#</packageName      >\n");
				// java 쓰기
				...
				while (st.hasMoreTokens()) {
					...
					// properties 쓰기
					propertiesKoWriter
							.write("category" + catNum + "Component" + comNum + "de" + depNum + "=" + dep + "\n");
					// properties 영문 쓰기
					propertiesEnWriter
							.write("category" + catNum + "Component" + comNum + "de" + depNum + "=" + dep + "\n");
					// xml 쓰기
					xmlWriter.write("\t\t\t<dependencyPackage>#" + "category" + catNum + "Component" + comNum + "de"
							+ depNum + "#</dependencyPackage>\n");
					// java 쓰기
					javaWriter.write(
							"\tpublic static String category" + catNum + "Component" + comNum + "de" + depNum + ";\n");
				}
				if (category_Component_d01.length() == 0) {
					...
					// xml 쓰기
					xmlWriter.write("\t\t\t<dependencyPackage>#" + "category" + catNum + "Component" + comNum
				...
				propertiesKoWriter
						.write("category" + catNum + "Component" + comNum + "add=" + category_Component_add + "\n\n");
				// properties 영문 쓰기
				propertiesEnWriter
				...
				propertiesEnWriter
						.write("category" + catNum + "Component" + comNum + "add=" + category_Component_add + "\n\n");
				// xml 쓰기
				xmlWriter.write("\t\t\t<description      >#" + "category" + catNum + "Component" + comNum
						+ "des#</description      >\n");
			...
			// java 닫기
			javaWriter.write("\n\tstatic {\n");
			javaWriter.write("\t\tNLS.initializeMessages(BUNDLE_NAME2, EgovComponentsInfo.class);\n");
			javaWriter.write("\t}\n");
