...
public class NewEgovCommngtWizard extends BasicNewResourceWizard implements INewWizard, IExecutableExtension {
	...
	public NewEgovCommngtWizard() {
		...
		setDefaultPageImageDescriptor(
				EgovCommngtPlugin.getDefault().getImageDescriptor(EgovCommngtPlugin.IMG_COMMNGT_WIZ_BANNER));
...
//		getShell().setBounds(rect.width/2-265, rect.height/2-320, 535, 640);
	...
	public boolean performFinish() {
		...
		try {
			...
			if (webExist) {
				/*
				 * MessageDialog dg = new MessageDialog ( getShell(),
				 * ComMngtMessages.newEgovCommngtWizard5, null,
				 * ComMngtMessages.newEgovCommngtWizard6, MessageDialog.QUESTION_WITH_CANCEL,
				 * new String[]{ "Yes", "No"}, 0 );
				 * 
				 * switch(dg.open()) { case 0: //yes context.setWebModify(true); webFile =
				 * "Action"; System.out.println("********** 0 : "+ webFile + " = " +
				 * context.isWebModify()); break; case 1: //no context.setWebModify(false);
				 * webFile = "Action"; System.out.println("********** 1 : "+ webFile + " = " +
				 * context.isWebModify()); break; default: // break; }
				 * 
				 * result = false;
				 */
				if (!MessageDialog.openConfirm(getShell(), ComMngtMessages.newEgovCommngtWizard5,
						ComMngtMessages.newEgovCommngtWizard6)) {
					return false;
				} else {
...
//		datasoucePage.performOk(); //프로퍼티파일에 설정
	...
	public boolean canFinish() {
		...
		if ((context.getJavaProject() != null || projectPage.isPageComplete()) && componentPage.isPageComplete()
				&& installTypePage.isPageComplete() && installTypePage.isSelectInstallTypePage != null) { // default,
																											// recommand
			successFinish = true;
//			return true;//finish 활성화
			...
			if ((context.getJavaProject() != null || projectPage.isPageComplete()) && componentPage.isPageComplete()
					&& datasoucePage.isPageComplete() && datasoucePage.checkLastPage != null) {
				successFinish = true;
//				return true;
			}
