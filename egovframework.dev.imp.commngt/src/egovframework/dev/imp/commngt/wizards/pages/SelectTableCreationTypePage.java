...
//	private int checkedComponentNum;
	
//	public String isCheckedComponentNumChanged = null;
	
//	private CustomizeTableCreationPage custPage = null;
     ...
     * @param context
     */
	public SelectTableCreationTypePage(String pageName, NewEgovCommngtContext context) {
		super(pageName);
		this.context = context;
		setTitle(ComMngtMessages.selectInstallTypePagepageTitle);
		setDescription(ComMngtMessages.selectInstallTypePagepageDescription);
	}
	...
	public void tableInfo(Composite parent) {
		...
		relTable.setText(ComMngtMessages.selectInstallTypePage0);
		...
		col2.setText(ComMngtMessages.selectInstallTypePage1);
		col2.setWidth(140);
		TableColumn col3 = new TableColumn(viewer.getTable(), SWT.NONE);
		col3.setText(ComMngtMessages.selectInstallTypePage2);
		col3.setWidth(300);
	...
	public void radioInfo(Composite parent) {
		...
		description.setText(ComMngtMessages.selectInstallTypePage4);
		...
		descript.setText(ComMngtMessages.selectInstallTypePagetypeDesc1);
	...
	SelectionListener selLis = new SelectionAdapter() {
		public void widgetSelected(SelectionEvent even) {
			...
			if(radioButton.equals(button)){
				context.setInstallType(ComMngtMessages.selectInstallTypePagetypeCode1);
				descript.setText(ComMngtMessages.selectInstallTypePagetypeDesc1);
				HandlingPropertiesUtil.setContext(context);
			}else if(radioButton2.equals(button)){
				context.setInstallType(ComMngtMessages.selectInstallTypePagetypeCode3);
				descript.setText(ComMngtMessages.selectInstallTypePagetypeDesc3);
...
//			isCheckedComponentNumChanged = null;
			flag = false;
			...
			//이전 페이지에서 컴포넌트를 추가적으로 설치하였을 경우에
//			if(checkedComponentNum != '0' && checkedComponentNum != checkedComponent.size() && !radioButton.getSelection()){
//				//Finish 버튼 비활성화
//				System.out.println(checkedComponentNum);
//				isCheckedComponentNumChanged = "isCheckedComponentNumChanged";
//			}
			
			int size = 0;
			for(int i = 0; i < checkedComponent.size(); i++){
				if(checkedComponent.get(i).getUseTable().indexOf(ComMngtMessages.selectTableCreationTypePagetableYN) > -1){
					size++;
				}
//				if(isCheckedComponentNumChanged == null && checkedComponentNum != checkedComponent.size()){
//					checkedComponentNum++;
//				}
			}
...
//			if(checkedComponent.get(0).getSuccessOrFail().length() == 0){
//				flag = false;
//				
//			}
...
//			if(isCheckedComponentNumChanged != null){
