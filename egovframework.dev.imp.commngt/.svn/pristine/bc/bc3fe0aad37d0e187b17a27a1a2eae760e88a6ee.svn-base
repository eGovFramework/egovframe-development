...
import egovframework.dev.imp.commngt.util.FilteredCheckboxTree;
import egovframework.dev.imp.commngt.util.FilteredCheckboxTree.FilterableCheckboxTreeViewer;
import egovframework.dev.imp.commngt.util.FormBrowser;
import egovframework.dev.imp.commngt.wizards.model.Category;
import egovframework.dev.imp.commngt.wizards.model.Component;
import egovframework.dev.imp.commngt.wizards.model.ComponentElementFactory;
...
public class SelectCommonComponentPage extends WizardPage implements ModifyListener{
     ...
     * 생성자
     */
	public SelectCommonComponentPage(String pageName, NewEgovCommngtContext context) {
		super(pageName);
		this.context = context;
		setTitle(ComMngtMessages.selectComponentPagepageTitle);
		setDescription(ComMngtMessages.selectComponentPagepageDescription);
...
//		else	setErrorMessage(ComMngtMessages.selectComponentPage_pageDescription);
	...
	public void createSelectComponentControls(Composite container) {
		...
		label.setText(ComMngtMessages.selectComponentPage3);
		...
		descriptionGroup.setText(ComMngtMessages.selectComponentPage0);
		...
		dependencyGroup.setText(ComMngtMessages.selectComponentPage1);
		...
		label1.setText(ComMngtMessages.selectComponentPage2);
		...
		tmptmp4.setText(ComMngtMessages.selectComponentPage7);
		...
		tmp_desc1.setText(ComMngtMessages.SelectComponentPage_4);
		...
		tmp_desc2.setText(ComMngtMessages.SelectComponentPage_5);
		...
		tmp_desc3.setText(ComMngtMessages.SelectComponentPage_6);
		...
		*/
		...
		tmptmp2.setImage(EgovCommngtPlugin.getDefault()
		        .getImage(EgovCommngtPlugin.IMG_COMMNGT_REMARKS));
		GridData tmp_gd12 = new GridData();
		tmp_gd12.horizontalSpan = 2;
	...
	private class componentSelectionChangedListener implements ISelectionChangedListener {
		...
		public void selectionChanged(SelectionChangedEvent event){
			if(event.getSelection() == null) return;
			...
			if(event.getChecked()){
				...
				//---------------------------------------------	
						if(eventObj instanceof Category){
							...
							if(exist_cnt > 0){
								MessageDialog.openInformation(getShell(), ComMngtMessages.selectComponentPagedialogTitle,ComMngtMessages.selectComponentPagedialog1);
							}
				...
				if(event.getElement() instanceof Category){
					...
					//---------------------------------------------		
							...
							if(treev.getGrayed((Category)eventObj)){
								...
								if(msg){
									//메시지는 자식노드일때와 다름
									MessageDialog.openInformation(getShell(), ComMngtMessages.selectComponentPagedialogTitle, ComMngtMessages.selectComponentPagedialog3 + " "+ cnt + ComMngtMessages.selectComponentPagedialog4); //$NON-NLS-1$
								}
							}
					...
					//---------------------------------------------	
							...
							if(((Component)eventObj).isCreatedComponent()){
								...
								if(checkDependComponent((Component)eventObj, ((Component)eventObj).getDependencyPackage()) > 0){
									MessageDialog.openInformation(getShell(), ComMngtMessages.selectComponentPagedialogTitle, ComMngtMessages.selectComponentPagedialog5 + " " + descDialog + ComMngtMessages.selectComponentPagedialog2); //$NON-NLS-1$
									treev.setChecked((Component)eventObj, true);
									//직접 체크 여부 - setSelection
...
//----------------성능향상 0929--------------------
				...
				if(cat != null){
					...
					for(int i=0; i < cat.getChildren().size(); i++){
						...
						if(i>0) children_before = cat.getChildren().get(i-1);
	...
	public int checkDependComponent(Component checkedComponent, List<Component> exceptDependency) {
		...
		if(exceptDependency != null){
			for(Component exCom : exceptDependency){
				if(!exCom.isSelection()) exceptDependency_false.add(exCom);
			}
		}
					...
					if(((Component)o).getDependencyPackage().contains(checkedComponent)){
						...
						if(exceptDependency != null ){
							if( !exceptDependency_false.contains(((Component)o)) ){
								cnt++;
								if(cnt==1) descDialog = ((Component)o).getName();
								else descDialog = descDialog + ", " + ((Component)o).getName(); //$NON-NLS-1$	
							}
	...
	public Object[] getCheckedElements_all(){
		...
		for(IComponentElement item : items){
			...
			for( IComponentElement comp : components){	
				if ( comp instanceof Component){
					if(treev.getChecked(comp)) checkedElements.add(comp);
				}
			}
		}
...
//		else context.setAddedOptions(false);
		...
		if(context.getComponent()== null || context.getComponent().size()==0){
			setErrorMessage(ComMngtMessages.selectComponentPagepageDescription);
		}
		else{
			setPageComplete(true);
	        setErrorMessage(null);
