...
public class BatchLog {
	...
	private static IStatus createStatus(int severity, int code, String message, Throwable exception) {
		IStatus status = new Status(severity, EgovBatchPlugin.PLUGIN_ID, code, message, exception);
		return status;
	}
	...
	private static void log(IStatus status) {
		EgovBatchPlugin.getDefault().getLog().log(status);
		return;
	}
	...
	public static void setDialogMessage(Throwable exception) {
		...
		StatusManager.getManager().handle(createStatus(IStatus.ERROR, IStatus.OK, exception.getMessage(), exception), StatusManager.LOG | StatusManager.BLOCK);
	}
}
