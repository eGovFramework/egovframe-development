...
public class BatchJobLauncherCustomizePage extends WizardPage {	
	 ...
	 *
	 */
	public BatchJobLauncherCustomizePage(String pageName,
			BatchJobLauncherContext context) {
		super(pageName);
	...
	private void setIsJobRepositoryIDExist(){
		...
		LinkedHashMap<String, String> beanList = FindXMLFileBeanIdValueUtil.findingWantedBeanInXMLFiles(context, "/beans/bean", "id", "jobRepository", "class", false); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
		...
		if(isJobRepositoryIDExist){
			LinkedHashMap<String, String> beanDataSourceRefList = FindXMLFileBeanIdValueUtil.findingWantedBeanInXMLFiles(context, "/beans/bean", "id", "jobRepository", "p:dataSource-ref", false); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
		...
		LinkedHashMap<String, String> beanValueTMList = FindXMLFileBeanIdValueUtil.findingWantedBeanInXMLFiles(context, "/beans/bean", "id", "transactionManager", "id", false); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
	...
	Listener validation = new Listener() {
		...
		public void handleEvent(Event event) {
			...
			errorMessage = getBeanIDTextErrorMessage(BatchMessages.BatchJobLauncherCustomizePage_ERROR_LAUNCHER_ID, launcherIdField.getText());
			if(!NullUtil.isEmpty(errorMessage)){
				setErrorMessage(errorMessage);
				return;
			}
			
			errorMessage = getBeanIDTextErrorMessage(BatchMessages.BatchJobLauncherCustomizePage_ERROR_OPERATOR_ID, operatorIdField.getText());
			if(!NullUtil.isEmpty(errorMessage)){
				setErrorMessage(errorMessage);
				return;
			}
			
			errorMessage = getBeanIDTextErrorMessage(BatchMessages.BatchJobLauncherCustomizePage_ERROR_EXPLORER_ID, explorerIdField.getText());
			if(!NullUtil.isEmpty(errorMessage)){
				setErrorMessage(errorMessage);
				return;
			}
			
			errorMessage = getBeanIDTextErrorMessage(BatchMessages.BatchJobLauncherCustomizePage_ERROR_REGISTER_ID, registerIdField.getText());
			if(!NullUtil.isEmpty(errorMessage)){
				setErrorMessage(errorMessage);
				return;
			}
			...
			if(!isJobRepositoryIDExist){
				...
				if(!memoryType.getSelection()){
					errorMessage = getBeanIDTextErrorMessage(BatchMessages.BatchJobLauncherCustomizePage_ERROR_DATASOURCE_BEAN_ID, datasourceBeanIDText.getText());
					if(!NullUtil.isEmpty(errorMessage)){
						setErrorMessage(errorMessage);
						return;
					}
		...
		private String getBeanIDTextErrorMessage(String beanIDName, String beanIDValue){
			if(NullUtil.isEmpty(beanIDValue)){
				...
				if(!NullUtil.isEmpty(duplicateBeanID)){
					return BatchMessages.BatchJobLauncherCustomizePage_DUPLICATE_BEAN_ID_1+duplicateBeanID+BatchMessages.BatchJobLauncherCustomizePage_DUPLICATE_BEAN_ID_2;
				}
			}
