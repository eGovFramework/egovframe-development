...
/**
 * 코드 생성 로그를 Console로 출력하는 유틸 클래스
 * <p><b>NOTE:</b> 로그 처리. 
 * @author 개발환경 개발팀 윤수열
 * @since 2009.08.10
 * @version 1.0
		...
		if (consoles.length>0){
			for (int i=0;i<consoles.length;i++){
				if (consoles[i] instanceof MessageConsole){
					if (((MessageConsole) consoles[i]).getName().equals("Model Based Code Generation Log"))
						console = (MessageConsole) consoles[i];
				}
			}
		}
		if (console==null){
			console = new MessageConsole("Model Based Code Generation Log",null);
