 ...
 */
package egovframework.bdev.imp.confmngt.common;
...
public class BConfMngtLog {
    ...
    private static IStatus createStatus(int severity, int code, String message,
            Throwable exception) {
        IStatus status =
            new Status(severity, EgovBatchConfMngtPlugin.PLUGIN_ID, code, message,
                exception);
        return status;
    }
    ...
    private static void log(IStatus status) {
    	EgovBatchConfMngtPlugin.getDefault().getLog().log(status);
        return;
    }
