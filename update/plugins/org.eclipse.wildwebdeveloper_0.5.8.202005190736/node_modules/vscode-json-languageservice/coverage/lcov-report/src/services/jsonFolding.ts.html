...
<head>
    <title>Code coverage report for src/services/jsonFolding.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">src/services</a> jsonFolding.ts</h1>
        <div class='clearfix'>
	...
	while (token !== SyntaxKind.EOF) {
		switch (token) {
			...
			case SyntaxKind.CloseBracketToken: {
				...
				<span class="missing-if-branch" title="else path not taken" >E</span>if (stack.length &gt; 0 &amp;&amp; stack[stack.length - 1].kind === kind) {
...
&nbsp;
			case SyntaxKind.BlockCommentTrivia: {
				...
				if (scanner.getTokenError() === ScanError.UnexpectedEndOfComment &amp;&amp; startLine + 1 &lt; document.lineCount) {
					...
					<span class="missing-if-branch" title="else path not taken" >E</span>if (startLine &lt; endLine) {
...
<span class="cstat-no" title="statement not covered" >							i--;</span>
						...
						<span class="missing-if-branch" title="else path not taken" >E</span>if (i &gt;= 0) {
							const range = stack[i];
							stack.length = i;
							<span class="missing-if-branch" title="else path not taken" >E</span>if (line &gt; range.startLine &amp;&amp; prevStart !== range.startLine) {
								range.endLine = line;
								addRange(range);
								prevStart = range.startLine;
							}
...
&nbsp;
	...
	<span class="missing-if-branch" title="if path not taken" >I</span>if (context &amp;&amp; context.onRangeLimitExceeded) {
<span class="cstat-no" title="statement not covered" >		context.onRangeLimitExceeded(document.uri);</span>
	}
&nbsp;
	const counts: number[] = [];
	for (let level of nestingLevels) {
		<span class="missing-if-branch" title="else path not taken" >E</span>if (level &lt; 30) {
	...
	for (let i = 0; i &lt; ranges.length; i++) {
		...
		<span class="missing-if-branch" title="else path not taken" >E</span>if (typeof level === 'number') {
