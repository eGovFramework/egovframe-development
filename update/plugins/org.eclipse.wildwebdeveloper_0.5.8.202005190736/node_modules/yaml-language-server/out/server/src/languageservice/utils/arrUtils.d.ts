export declare function removeDuplicates(arr: any, prop: any): any[];
export declare function getLineOffsets(textDocString: String): number[];
export declare function removeDuplicatesObj(objArray: any): any[];
export declare function matchOffsetToDocument(offset: number, jsonDocuments: any): any;
export declare function matchOffsetToDocument2(offset: number, jsonDocuments: any): any;
export declare function filterInvalidCustomTags(customTags: String[]): String[];
