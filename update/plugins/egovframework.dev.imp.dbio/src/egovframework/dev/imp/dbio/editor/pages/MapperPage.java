...
import egovframework.dev.imp.dbio.editor.MapperEditor;
import egovframework.dev.imp.dbio.editor.parts.MapperMasterDetailsBlock;
...
public class MapperPage extends SimpleFormPage implements Listener {
	
	private MapperMasterDetailsBlock mdBlock;
	private final MapperEditor editor;
	private List<?> queryTestResult;
	private boolean voCreationStatus = false;
	
	public MapperPage(MapperEditor editor) {
		super(editor, MapperPage.class.getName(), "Mapper");
		this.setMdBlock();
		this.editor = editor;
	}
	
	private void setMdBlock() {
		this.mdBlock = new MapperMasterDetailsBlock(this);
	}
	...
	public String openOrCreateNewJavaClass(String className) {
		...
		if(getVoCreationStatus() && queryTestResult == null) {
			MessageDialog.openInformation(this.getSite().getShell(),
					"Create VO", DbioMessages.mapper_info_doQueryTest);
			return null;
		}
...
//		}
	 ...
	 */
	public MapperMasterDetailsBlock getMdBlock() {
		return this.mdBlock;
	}
  	/**
	 * MapperEditor 객체를 리턴함 
	 */
	public MapperEditor getEditor() {
		return this.editor;
	}
