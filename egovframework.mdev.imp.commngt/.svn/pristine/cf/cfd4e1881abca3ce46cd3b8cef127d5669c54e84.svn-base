...
import egovframework.mdev.imp.commngt.common.MobileComponentsInfo;
...
public final class ComponentElementFactory {
	...
	private void initializeFactory() throws MalformedURLException, JDOMException, IOException, SecurityException, NoSuchFieldException, InstantiationException, IllegalAccessException {
		...
		//Document doc = builder.build(MobilePluginLocationFinder.getAbsoluteFile(MobileCommngtPlugin.getDefault(), MobileComponentsInfo.componentElementFactoryPath ));
		Document doc = builder.build( FileLocator.toFileURL(MobileComMngtPlugin.getDefault().getBundle().getEntry(MobileComMngtMessages.componentElementFactoryPath)) );
		Element components = doc.getRootElement();
		List<?> clist = components.getChildren("category"); //$NON-NLS-1$
		Class<MobileComponentsInfo> cls = MobileComponentsInfo.class;
	...
	/** 필드 값 읽기 */
	private String getFieldValue(Class<MobileComponentsInfo> c, Element e, String a, String flag){
		
		try {
			Field f = getFieldInfo(c, e, a, flag);
			if(f != null) return f.get( f ).toString();
		} catch (IllegalArgumentException e1) {
			MobileComMngtLog.logError(e1);
	...
	/** 필드 정보 얻기 */
	private Field getFieldInfo(Class<MobileComponentsInfo> c, Element e, String a, String flag){
		try {
	...
	/** 필드 값 문자열 얻기 */
	private String getFieldValueString(Class<MobileComponentsInfo> c, String a){
		try {
			if(a != null && a != "")
	...
	/** 컴포넌트 정보 얻기 */
	protected List<IComponentElement> getComponent(Class<MobileComponentsInfo> cls, Element category, List<Component> totalComponent, String catName) {
	...
	/** 컴포넌트 디펜던시 정보 얻기 */
	private List<Component> computeDependency(Class<MobileComponentsInfo> cls, Element element, List<Component> totalComponent){
