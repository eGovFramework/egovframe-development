...
public class ComResourceUtils{
     ...
     * @throws CoreException
     */
    public static IFile copyWebXmlFile(MobileCommngtContext context, IContainer container, String fname, InputStream content, IProgressMonitor monitor, String type) throws CoreException {
    	IPath p = new Path(fname.replace('\\', '/'));
    	IContainer fo = null;
    ...
    public static InputStream modifyWebXml(IContainer container, String fname, InputStream newCon, InputStream oriCon, IProgressMonitor monitor){
    	...
    	try {
    		...
    		if(mappingExist) {
    			...
    			String urlMdoPattern = "*.mdo";
    			
    			// 새로운 컨텐츠에서 *.mdo에 대한 url servlet 및 context-param작업을 수행한다.
    			// *.mdo url패턴에 대하여 해당 노드가 존재할 경우 *.mdo가 가리키는 servlet 노드를 가져온다.(새로운 컨텐츠)
    			Node mdoNode = getServlet(newRootNode, urlMdoPattern);
    			
    			// *.mdo url패턴에 대하여 해당 노드가 존재할 경우 *.mdo가 가리키는 servlet 노드를 가져온다.(기존)
    			Node mdoOldNode = getServlet(oriRootNode, urlMdoPattern);
    			
    			// servlet node를 이용하여 init-param/param-value를 수정한다.
    			if(mdoOldNode != null) {
    				// 기존에 *.mdo가 있다면 병합, 없다면 새로운 servlet 노드를 추가한다.
    				mergeServlet(mdoOldNode, mdoNode);
    			} else {
    				//기존에 *.mdo 노드가 없다면 최초의 servlet과 새로운 servlet의 병합을 실시한다.
    				NodeList servletList = XmlUtil.getNodeList(oriRootNode, "/web-app/servlet");
    				if(servletList.getLength()>0) {
    					mergeServlet(servletList.item(0), mdoNode);
    					//기존에 *.mdo노드가 없다면 새로운 servlet-mapping(*.mdo)를 추가한다. 추가시 기존의 servletName을 적용한다.
    					XmlUtil.addNode(oriRootNode, "/web-app", nodeToString(getServletMapping(newRootNode, urlMdoPattern, XmlUtil.getNode(servletList.item(0), "./servlet-name").getFirstChild().getNodeValue())), "\t", "\n");
    				}
    			}
    			
    			// *.mdo 패턴에 대한 context를 가져온다.
    			if(mdoNode != null) {
    				Node mdoOldContextParam = getContextParam(oriRootNode, mdoOldNode);	//기존
    				Node mdoContextParam = getContextParam(newRootNode, mdoNode);			//새로운 컨텐츠
    				
    				//context node를 이용하여 param-value를 수정한다.
    				mergeContext(mdoOldContextParam, mdoContextParam);
    			}
...
//    									} else {
     ...
     * @throws CoreException
     */
    public static void copyDispatcherServletFile(MobileCommngtContext context, IContainer container, String fname, InputStream content, IProgressMonitor monitor, ComResourceUtilVO comVo) throws CoreException {
