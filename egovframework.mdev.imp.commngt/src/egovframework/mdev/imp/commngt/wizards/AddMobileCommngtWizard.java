...
public class AddMobileCommngtWizard extends BasicNewResourceWizard implements INewWizard, IExecutableExtension {
	...
	public AddMobileCommngtWizard() {
		...
		setDefaultPageImageDescriptor(
				MobileComMngtPlugin.getDefault().getImageDescriptor(MobileComMngtPlugin.imgMCommngtWizBanner));
...
//		getShell().setBounds(rect.width/2-265, rect.height/2-320, 535, 640);
	...
	public void addPages() {
		...
		if (projectPageView) {
			projectPage = new SelectTargetProjectPage(MobileComMngtMessages.addMobileCommngtWizard1, javaProject,
					getContext());
			addPage(projectPage);
		} else {
	...
	public boolean performFinish() {
		...
		try {
			...
			if (webExist) {
				MessageDialog dg = new MessageDialog(getShell(), MobileComMngtMessages.addMobileCommngtWizard5, null,
						MobileComMngtMessages.addMobileCommngtWizard6, MessageDialog.QUESTION_WITH_CANCEL,
						new String[] { "Yes", "No" }, 0);
