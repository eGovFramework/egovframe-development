...
/**
 * Mapper 에디터의 아웃라인 페이지의 컨텐트 프로바이더 클래스 
 * @author 개발환경 개발팀 김효수
 * @since 2019.02.25
 * @version 1.0
 * @see
...
public class MapperOutlineContentProvider implements ITreeContentProvider{
		
		/**
		 * 생성자 
		 */
		public MapperOutlineContentProvider() {
			super();
		}
...
//        	System.out.println("getAdapter()");
		...
		private Object[] getGroups(Element element) {
			...
			/*
				Object[] groups = new MapperGroupElement[4];
				groups[0] = new MapperQueryGroupElement(element);
				groups[1] = new MapperAliasGroupElement(element);				
				groups[2] = new MapperParameterMapGroupElement(element);
				groups[3] = new MapperResultMapGroupElement(element);
			*/
				Object[] groups = new MapperGroupElement[2];
				groups[0] = new MapperQueryGroupElement(element);
				groups[1] = new MapperResultMapGroupElement(element);		
