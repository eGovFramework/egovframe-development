...
public class JobParameterDialog extends StatusDialog {
	 ...
	 * @param jobParameter
	 */
	public JobParameterDialog(Shell shell, boolean isAddButton,
			List<String> existingIdList, JobParameterInfo jobParameter) {
		super(shell);
	...
	private void createDataTypeField(Composite control){
		String[] list = new String[] {JobParameterComboString.DEFAULT, JobParameterComboString.STRING, JobParameterComboString.LONG, JobParameterComboString.DATE, JobParameterComboString.DOUBLE}; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
	...
	Listener validation = new Listener(){
		public void handleEvent(org.eclipse.swt.widgets.Event event) {
			...
			if(JobParameterComboString.DATE.equals(dataTypeField.getText()) && NullUtil.isEmpty(dateFormatField.getText())){
	...
	private boolean valueValidation(String value){
		...
		if(JobParameterComboString.STRING.equals(dataTypeString)){
			if(!(value.indexOf("\\") > -1) && !(value.indexOf("\"") > -1) && !StringUtil.hasKorean(value) && !StringUtil.hasEmptySpace(value)){ //$NON-NLS-1$ //$NON-NLS-2$
				return true;
	...
	private HashMap<String, String> getFormatList(){
		...
		formatList.put(JobParameterComboString.DATE_FORMAT_YYYY_dash_MM_dash_DD, JobParameterComboString.DATE_FORMAT_REGEX_DASH);
		formatList.put(JobParameterComboString.DATE_FORMAT_YYYY_slash_MM_slash_DD, JobParameterComboString.DATE_FORMAT_REGEX_SLASH);
		formatList.put(JobParameterComboString.DATE_FORMAT_YYYY_dot_MM_dot_DD, JobParameterComboString.DATE_FORMAT_REGEX_DOT);
		formatList.put(JobParameterComboString.DATE_FORMAT_YY_dash_MM_dash_DD, JobParameterComboString.DATE_FORMAT_REGEX_DASH);
