 ...
 */
package egovframework.bdev.imp.batchnature.common;
...
public class BatchNatureLog {
	...
	private static IStatus createStatus(int severity, int code, String message,
			Throwable exception) {
		IStatus status = new Status(severity, EgovBatchNaturePlugin.PLUGIN_ID, code,
				message, exception);
		return status;
	}
	...
	private static void log(IStatus status) {
		EgovBatchNaturePlugin.getDefault().getLog().log(status);
		return;
	}
